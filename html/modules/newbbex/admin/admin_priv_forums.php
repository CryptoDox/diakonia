...
<?php
			while ($row = $xoopsDB->fetchArray($result))
			{
				...
				$post_toggle_link = XOOPS_URL."/modules/newbbex/admin/admin_priv_forums.php?forum=$forum&op_userid=".$row['uid']."&op=";
				if ($row['can_post'])
				{
				...
				$remove_link = myTextForm(XOOPS_URL."/modules/newbbex/admin/admin_priv_forums.php?forum=$forum&amp;op=deluser&amp;op_userid=".$row['uid'], _MDEX_A_REMOVE);
?>
								<tr>
