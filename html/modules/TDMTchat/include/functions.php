...
$xoTheme->addScript(XOOPS_URL."/modules/".$xoopsModule->dirname()."/js/jquery.tchat.js");

$xoTheme->addStylesheet(XOOPS_URL."/modules/".$xoopsModule->dirname()."/css/tdmtchat.css");
...
$tchat_handler =& xoops_getModuleHandler('tdmtchat_tchat', 'TDMTchat');
	...
	$tocriteria->add(new Criteria('tchat_from',  $uname));
	...
	$arr2 =& $tchat_handler->getAll($criteria2);
	$ret = array();
	foreach (array_keys($arr2) as $tree) {	
	//$reta = $arr2[$tree]->getVar('tchat_id');
	$ret[$tree] = &$arr2[$tree];
	}
...
function tdmtchat_history()
{
global $xoopsModuleConfig;

$tchat_handler =& xoops_getModuleHandler('tdmtchat_tchat', 'TDMTchat');
	
	$time = time() - ($xoopsModuleConfig['tdmtchat_history'] * 24 * 60 * 60);
	
	$criteria = new CriteriaCompo();	
	$criteria->add(new Criteria('tchat_sent', $time, '<'));	
	$arr2 =& $tchat_handler->getAll($criteria);
	foreach (array_keys($arr2) as $tree) {	
	$obj =& $tchat_handler->get($arr2[$tree]->getVar('tchat_id'));
	$tchat_handler->delete($obj);
	}
 ...
 function tdmtchat_theone($arr) {
 
	foreach (array_keys($arr) as $tree) {	
	$ret = $arr[$tree]->getVar('tchat_id');
	}
 ...
 function tdmtchat_PrettySize($size)
{
    $mb = 1024 * 1024;
    if ($size > $mb)
    {
