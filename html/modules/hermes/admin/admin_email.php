...
global $xoopsModule, $xoopsDB,$adoMailingListe;
  ...
  echo _JJD_JSI_SPIN;  
    ...
    while ($sqlfetch = $xoopsDB->fetchArray($sqlquery)) {
      ...
      echo '<tr>';
      echo "<td{$bg} align='right'>({$idListe})</td>";      
      ...
      //echo "<td>[{$sqlfetch['editBeforeSend']}]</td>";        
        //-----------------------------------------------------------------------   	   
        $link = "admin_email.php?op=edit&idListe=".$idListe;
        echo build_icoOption($link, _JJDICO_EDIT, _AD_HER_EDIT, '', '', $bg);      
        //-----------------------------------------------------------------------
        //Dupliquer 
      	$link = "admin_email.php?op=duplicate&idListe=".$idListe;     
        echo build_icoOption($link, _JJDICO_DUPLICATE, _AD_HER_DUPLICATE, '', '', $bg);           
         
        //-----------------------------------------------------------------------
    	  $link = "admin_email.php?op=remove&idListe={$idListe}&name={$sqlquery['nom']}";
...
echo "<TABLE BORDER=0 CELLPADDING=2 CELLSPACING=3>
  <tr valign='top'>
    ...
    <td align='right'>
    <input type='button' name='new' value='"._AD_HER_NEW."' onclick='".buildUrlJava("admin_email.php?op=new",false)."'>    
  </tr>
  </form>";
  ...
  echo _JJD_JSI_SPIN;  
    ...
    //********************************************************************
  	...
  	//echo "<B>"._AD_HER_LISTE_MANAGEMENT."</B></div>";
 		...
 		echo "<FORM ACTION='admin_email.php?op=save' METHOD=POST>";
    ...
    //---Name
    echo buildInput(_AD_HER_NAME_LIST, '', 'txtName', $myts->makeTboxData4Show($p['nom'], "1", "1", "1"), '60%');    
    ...
    */
    //---texte    
   	...
   	$value = $p['liste'];
   	$desc1 = getEditorHTML(99, $value, $name = 'txtListe', $caption = 'Liste',  
                   $width='80%', $height='400px', 
                   $rows = 24 , $cols = 69 );

    echo "<TR>"._br;
    echo "<TD align='center' ><B>"._AD_HER_EMAIL_LIST."</B</TD>"._br;    
    echo "<TD align='left'  >";
    echo $desc1->render();
    echo "</TD>"._br;
    echo "</TR>"._br;
...
*/
    ...
    echo "<TABLE BORDER=0 CELLPADDING=2 CELLSPACING=3>
      <tr valign='top'>
        <td align='left' ><input type='button' name='cancel' value='"._CANCEL."' onclick='".buildUrlJava("admin_email.php",false)."'></td>
        <td align='left' width='200'></td>
...
$adoMailingListe = new cls_hermes_mailingListe();
  ...
  admin_xoops_cp_header(_HER_ONGLET_EMAIL, $xoopsModule); 

  switch($op) {
		
  case "save":
		$adoMailingListe->saveRequest ($_POST);
    redirect_header("admin_email.php?op=list",1,_AD_HER_ADDOK);		
		break;
  ...
  case "duplicate":
		$p = $adoMailingListe->newClone ($idListe, true,'nom');    
    editMailingListe ($p);    
  	break;
