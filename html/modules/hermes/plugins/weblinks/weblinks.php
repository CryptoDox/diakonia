...
class cls_weblinks extends cls_hermes_plugin  {  
 ...
 * Constructucteur:
 ************************************************************/
function  cls_weblinks($options){
    cls_hermes_plugin::cls_hermes_plugin($options); 



  $options = array ('moduleName'  => 'weblinks',
                     'version'     => '1.01.01',
                     'pluginName'  => _WLINKS_NAME,
                     'description' => _WLINKS_PLUGIN_DESC,
                     'header'      => _WLINKS_HEADER,
                     'footer'      => _WLINKS_ALL_LINKS,
                     'identifiant' => 200);                                              

  $tColDef = array (array('ordreTitle',           _WLINKS_TITLE) ,
                          array('ordreSommaire',        _WLINKS_SUMMARY) ,
                          array('ordreDatePublication', _WLINKS_PUBLISHED) ,
                          array('ordreLecture',         _WLINKS_LECTURE) ,
                          array('ordreCatCreation',     _WLINKS_CATCREATED) ,                          
                          array('ordreCatLecture',      _WLINKS_CATLECTURE) ,                          
                          array('ordreCategorie',       _WLINKS_CATEGORIE));
...
function getInfoPluggin(&$tProperty, &$params){  
  ...
  $params = array ('ruptureMaitre' => array('name'        => _AD_HER_ORDREDETRI, 
                                      'value'       => 1, 
                                      'description' => _WLINKS_AFFICHAGE_DESC,
                                      'type'        => 2,
                                      'list'        => _WLINKS_AFFICHAGE_LIST),
                    'show_title' => array('name'    => 'show_title', 
                                      'value'       => 1, 
                                      'description' => _WLINKS_SHOWTITLE_DESC,
                                      'type'        => 2,
                                      'list'        => _AD_HER_NOYES),
                    'show_categorie' => array('name'=> 'show_categorie', 
                                      'value'       => 1, 
                                      'description' => _WLINKS_SHOWCATEGORIE_DESC,
                                      'type'        => 2,
                                      'list'        => _AD_HER_NOYES)
                    );
  //-------------------------------------------------------------
...
SELECT j.cid, j.title, j.imgurl, j.description, j.catdescription, j.orders, 
jjd.lid, jjd.title, jjd.url, jjd.banner, jjd.description, jjd.name, 
jjd.mail, jjd.company, jjd.addr, jjd.tel, jjd.time_create, 
jjd.time_update, jjd.hits, jjd.rating, jjd.votes, jjd.recommend, jjd.broken
FROM jjd02_weblinks_category j, 
jjd02_weblinks_catlink jj, 
jjd02_weblinks_link jjd
WHERE j.cid=jj.cid  AND jj.lid=jjd.lid
ORDER BY j.cid, jjd.title
*/
...
//-------------------------------------------------------------------
   ...
   while ($sqlfetch = $xoopsDB->fetchArray($sqlquery)) {
      ...
      $title = $myts->sanitizeForDisplay($sqlfetch['title'],1,0,0,0,0);      
      ...
      $title = $myts->sanitizeForDisplay($sqlfetch['name'],1,0,0,0,0);         
