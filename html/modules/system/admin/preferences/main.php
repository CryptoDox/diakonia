<?php
// $Id: main.php 3464 2009-08-17 20:37:03Z trabis $
//  ------------------------------------------------------------------------ //
...
if ( !is_object($xoopsUser) || !is_object($xoopsModule) || !$xoopsUser->isAdmin($xoopsModule->mid()) ) {
    ...
    if ($op == 'list') {
        ...
        $admin_mids = array_diff($admin_mids, array(1));
        if (count($admin_mids)) {
            ...
            foreach ($mods as $mod) {
                if ($mod->getVar('hasnotification') || is_array( $mod->getInfo('config') ) || is_array( $mod->getInfo('comments') )) {
                    $modules[$mod->getVar('mid')] = $mod->getVar('name');
                }
            }
        }
        if (count($modules)) {
    ...
    if ($op == 'showmod') {
        ...
        if (!empty($_GET["redirect"])) {
            $myts =& MyTextSanitizer::getInstance();
            $form->addElement(new XoopsFormHidden('redirect', $myts->htmlspecialchars($_GET["redirect"])));
