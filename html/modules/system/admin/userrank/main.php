<?php
// $Id: main.php 4027 2009-12-12 10:06:10Z trabis $
//  ------------------------------------------------------------------------ //
...
case "RankForumAdd":
    ...
    if (!$db->query($sql)) {
        ...
        if ( $errors = $uploader->getErrors() ) {
            redirect_header("admin.php?fct=userrank&amp;op=RankForumAdmin",3, xoops_error($errors) . _AM_DBUPDATED);
        } else {
...
case "RankForumSave":
    ...
    if (!$db->query($sql)) {
        ...
        if ($delete_old_image) {
            ...
            if (false === strpos($old_rank_path, XOOPS_UPLOAD_PATH) && is_file($old_rank_path)) {
                unlink($old_rank_path);
            }
        }
        if (!empty($_POST[$_POST['xoops_upload_file'][0]]) && $errors = $uploader->getErrors() ) {
            redirect_header("admin.php?fct=userrank&amp;op=RankForumAdmin",3, xoops_error($errors) . _AM_DBUPDATED);
        } else {
            redirect_header("admin.php?fct=userrank&amp;op=RankForumAdmin",3, _AM_DBUPDATED);
        }
