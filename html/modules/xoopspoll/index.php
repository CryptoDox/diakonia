...
$url    = XoopspollRequest::getString('url', '');
...
if (empty($pollId)) {
    ...
    $GLOBALS['xoopsTpl']->assign(array(
                            ...
                            'results_link_icon'   => $GLOBALS['xoopsModule']->getInfo('icons16') . DIRECTORY_SEPARATOR . "open12.gif",
...
{@internal DEBUG CODE
        ...
        $polls['pollVoters'] = intval($pollObj->getVar('voters'));
        $polls['pollVotes']  = intval($pollObj->getVar('votes'));
    ...
    $option_id = XoopspollRequest::getInt('option_id', 0, 'POST');
    $mail_author = false;
    $pollObj = $pollHandler->get($pollId);
    if ($pollObj instanceof XoopspollPoll) {
        if ($pollObj->getVar('multiple')) {
            $optionId = XoopspollRequest::getArray('option_id', array(), 'POST');
            ...
            $optionId = XoopspollRequest::getInt('option_id', 0, 'POST');
        }
...
//            $url = $GLOBALS['xoops']->buildUrl("index.php", array('poll_id' => $pollId));
    ...
    if (null != $url) {
        ...
        redirect_header($GLOBALS['xoops']->buildUrl("pollresults.php", array('poll_id' => $pollId)),
    ...
    if ($pollObj->hasExpired()) {
        redirect_header($GLOBALS['xoops']->buildUrl("pollresults.php", array('poll_id' => $pollId)),
                                               XoopspollConstants::REDIRECT_DELAY_SHORT,
                                               _MD_XOOPSPOLL_SORRYEXPIRED);
    }
    ...
    if ($pollObj->isAllowedToVote()) {
        ...
        $multiLimit = intval($pollObj->getVar('multilimit'));
