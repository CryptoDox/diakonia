 ...
 * @version         $Id: request.php 12482 2014-04-25 12:08:17Z beckmi $
 */

/**
 * Set the available masks for cleaning variables
 */
define('XOOPSPOLL_REQUEST_NOTRIM', 1);
define('XOOPSPOLL_REQUEST_ALLOWRAW', 2);
define('XOOPSPOLL_REQUEST_ALLOWHTML', 4);

/**
 * XoopspollRequest Class
 * This class serves to provide a common interface to access
 * request variables.  This includes $_POST, $_GET, and naturally $_REQUEST.  Variables
 * can be passed through an input filter to avoid injection or returned raw.
 */
class XoopspollRequest
{
    ...
    static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
    {
        ...
        if (isset($input[$name]) && $input[$name] !== null) {
            ...
            $var = XoopspollRequest::_cleanVar($input[$name], $mask, $type);
            // Handle magic quotes compatability
            if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
                $var = XoopspollRequest::_stripSlashesRecursive($var);
            }
        } elseif ($default !== null) {
            // Clean the default value
            $var = XoopspollRequest::_cleanVar($default, $mask, $type);
        } else {
            $var = $default;
        }
    ...
    {
        return XoopspollRequest::getVar($name, $default, $hash, 'int');
    }
    ...
    {
        return XoopspollRequest::getVar($name, $default, $hash, 'float');
    }
    ...
    {
        return XoopspollRequest::getVar($name, $default, $hash, 'bool');
    }
    ...
    {
        return XoopspollRequest::getVar($name, $default, $hash, 'word');
    }
    ...
    {
        return XoopspollRequest::getVar($name, $default, $hash, 'cmd');
    }
    ...
    static function getString($name, $default = '', $hash = 'default', $mask = 0)
    {
        ...
        return (string) XoopspollRequest::getVar($name, $default, $hash, 'string', $mask);
    }

    static function getArray($name, $default = array(), $hash = 'default')
    {
        return XoopspollRequest::getVar($name, $default, $hash, 'array');
    }

    static function getText($name, $default = '', $hash = 'default')
    {
        return (string) XoopspollRequest::getVar($name, $default, $hash, 'string', XOOPSPOLL_REQUEST_ALLOWRAW);
    }
    ...
    static function get($hash = 'default', $mask = 0)
    {
        ...
        $result = XoopspollRequest::_cleanVar($input, $mask);
        // Handle magic quotes compatability
        if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
            $result = XoopspollRequest::_stripSlashesRecursive($result);
        }
    ...
    static function set($array, $hash = 'default', $overwrite = true)
    {
        foreach ($array as $key => $value) {
            XoopspollRequest::setVar($key, $value, $hash, $overwrite);
        }
    }
    ...
    {
        XoopspollRequest::_cleanArray($_FILES);
        XoopspollRequest::_cleanArray($_ENV);
        XoopspollRequest::_cleanArray($_GET);
        XoopspollRequest::_cleanArray($_POST);
        XoopspollRequest::_cleanArray($_COOKIE);
        XoopspollRequest::_cleanArray($_SERVER);
        if (isset($_SESSION)) {
            XoopspollRequest::_cleanArray($_SESSION);
        }
        $REQUEST = $_REQUEST;
        $GET = $_GET;
        $POST = $_POST;
    ...
    static function _cleanVar($var, $mask = 0, $type = null)
    {
        ...
        if ($mask & 2) {
            ...
            if (is_null($safeHtmlFilter)) {
                $safeHtmlFilter = XoopspollFilterInput::getInstance(null, null, 1, 1);
            }
            $var = $safeHtmlFilter->clean($var, $type);
        } else {
            // Since no allow flags were set, we will apply the most strict filter to the variable
            if (is_null($noHtmlFilter)) {
                $noHtmlFilter = XoopspollFilterInput::getInstance( /* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
            }
    ...
    {
        $value = is_array($value) ? array_map(array('XoopspollRequest', '_stripSlashesRecursive'), $value) : stripslashes($value);
...
/**
 * XoopspollFilterInput is a class for filtering input from any data source
 * Forked from the php input filter library by: Daniel Morris <dan@rootcube.com>
 * Original Contributors: Gianpaolo Racca, Ghislain Picard, Marco Wandschneider, Chris Tobin and Andrew Eddie.
 *
 * @author      Louis Landry <louis.landry@joomla.org>
 */
class XoopspollFilterInput
{
     ...
     *      <pre>  $filter = & XoopspollFilterInput::getInstance();</pre>
     *
     * @static
     *
     * @param array $tagsArray  list of user-defined tags
     ...
     * @param int   $xssAuto    Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
     *
     * @return object The XoopspollFilterInput object.
     * @since   1.5
     */
    public function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
    {
        ...
        if (empty ($instances[$sig])) {
            $instances[$sig] = new XoopspollFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
        }
    ...
    public function clean($source, $type = 'string')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'STRING' :
                ...
                if (isset($this) && is_a($this, 'XoopspollFilterInput')) {
                    $filter =& $this;
                } else {
                    $filter = XoopspollFilterInput::getInstance();
                }
            ...
            default :
                // Check for static usage and assign $filter the proper variable
                if (is_object($this) && get_class($this) == 'XoopspollFilterInput') {
                    $filter =& $this;
                } else {
                    $filter = XoopspollFilterInput::getInstance();
                }
    ...
    protected function _cleanAttributes($attrSet)
    {
        ...
        for ($i = 0; $i < count($attrSet); ++$i) {
            ...
            if (XoopspollFilterInput::checkAttribute($attrSubSet)) {
                continue;
            }
