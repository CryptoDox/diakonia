...
<{  foreach item=option from=$block.options}>
    <tr class='<{cycle values="even,odd"}>'>
      ...
      <td class='left' style='margin-left: 1em;'>
        ...
        <div class='width90'><img src='<{$xoops_url}>/modules/xoopspoll/assets/images/colorbars/<{$option.color}>' style='height: 14px; width: <{$option.percent}>%;' alt='<{$option.percent}>' /></div>
      </td>
    </tr>
<{  /foreach}>
...
<{  foreach item=option from=$block.options}>
    <tr class='<{cycle values="even,odd"}>'>
      <td class='center'><input type='<{$block.optionType}>' name='<{$block.optionName}>' value='<{$option.id}>' /></td>
...
<{    if $block.showResultsLink && $block.visible && $block.votes}>
        <input type='button' value='<{$block.langResults}>' onclick="location='<{$xoops_url}>/modules/xoopspoll/pollresults.php?poll_id=<{$block.id}>'" />
<{    /if}>
      </td>
    </tr>
<{else}>
...
<{    else}>
        <a href='<{$xoops_url}>/modules/xoopspoll/pollresults.php?poll_id=<{$poll.id}>'><{$block.langComments}>?</a>
<{    /if}>
      </td>
    </tr>
<{  /if}>
