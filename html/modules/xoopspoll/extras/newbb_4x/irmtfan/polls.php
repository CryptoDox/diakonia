 ...
 * @version         $Id: polls.php 12482 2014-04-25 12:08:17Z beckmi $
 */
...
//            $poll_obj->setVar("end_time", userTimeToServerTime(strtotime($end_time), $timezone));
        ...
        $poll_obj->setVar("weight", intval(@$_POST['weight']));
        $poll_obj->setVar("multiple", intval(@$_POST['multiple']));
...
case "restart_ok":
    ...
    $end_time = (empty($_POST['end_time'])) ? 0 : intval($_POST['end_time']);
    if (!empty($end_time)) {
        $timezone = ($GLOBALS['xoopsUser'] instanceof XoopsUser) ? $GLOBALS['xoopsUser']->getVar("timezone") : null;
//        $poll_obj->setVar("end_time", userTimeToServerTime(strtotime($end_time), $timezone));
        //Hack by irmtfan
